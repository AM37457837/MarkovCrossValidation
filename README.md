...
Named chrX_200bp_bins.tsv. Columns: start (0-based), end, CTCF (B/U), REST (B/U), EP300 (B/U).
Example header:
``` text
...
Markov order m: 0–10.
```
``` Example workflow (for chr1, CTCF, m=6, k=5):
text
...
## 6. What the Script Does
 ...
 k-fold CV: Train m-order Markov on k-1 folds (pseudo-counts=1), score held-out (log P_bound - log P_unbound).
 Outputs: Console stats, results.png (ROC/PR curves + per-fold AUCs).

## 7. Outputs
 Console: Bin counts, fold progress.
 results.png: ROC (left) + PR (right) for k folds.
